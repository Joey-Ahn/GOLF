...
conn <- dbConnect(RSQLite::SQLite(), "score_smpl.sqlite")
#...........................................................................................................................................
get.score <- function() {
  # res <- dbSendQuery(conn, "SELECT * FROM score_db")
  res <- dbSendQuery(conn, "SELECT * FROM score_smpl")
  score <- dbFetch(res)
  dbClearResult(res)
...
score.insert.callback <- function(data, row) {
  ...
  query <- paste0("INSERT INTO score_smpl (Date, Time, Course, id) VALUES (",
...
score.update.callback <- function(data, olddata, row) {
  ...
  query <- paste0("UPDATE score_smpl SET ",
...
score.delete.callback <- function(data, row) {
  ...
  query <- paste0('DELETE FROM score_smpl WHERE id = ', data[row,]$id)
  dbSendQuery(conn, query)
  return(get.score())
}
############################################################################################################################################
server <- function(input, output) {
  ...
  dtedit(input, output,
         ...
         edit.cols = c("Date", "Time", "Course"),
         edit.label.cols = c("Date", "AM/PM", "Course"),
         input.types = c(Course='textInput'),
         # view.cols = names(score)[c(1, 2, 3, 5, 6, 7, 8, 9)],
         view.cols = names(score)[c(1, 2, 3)],
